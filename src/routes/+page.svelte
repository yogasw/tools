<script>
  ...
  $: filteredAndSortedTools = $tools.sort((a, b) => {
...
<div class="py-16 space-y-12">
  ...
  <div class="text-center space-y-6">
    ...
    <div class="max-w-xl mx-auto pt-4">
      <button
        on:click={() =>
          import("$lib/stores/searchModal.js").then((m) =>
            m.searchModal.open(),
          )}
      ...
      >
        ...
        <kbd
          class="px-2 py-1 text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[#0a0a0a] border border-gray-300 dark:border-gray-700 rounded"
        >
          {isMac ? "⌘" : "CTRL"} K
        </kbd>
      </button>
      <p class="text-xs text-gray-500 dark:text-gray-500 text-center mt-2">
        Press <kbd
          class="px-1 py-0.5 text-xs font-semibold bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded"
          >{isMac ? "⌘" : "CTRL"}</kbd
        >
        +
        <kbd
      ...
      >
        ...
        <div class="space-y-3 pr-8">
          ...
          <div class="flex items-center gap-3">
            ...
            <h3
              class="font-semibold text-gray-900 dark:text-white group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors flex-1"
            >
              {tool.name}
            </h3>
          </div>
          ...
          <div class="flex items-center gap-2 flex-wrap">
            {#if isBookmarked(tool.id)}
              <span
                class="px-2 py-0.5 text-xs font-medium text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 rounded"
              >
                Bookmarked
              </span>
            {/if}
