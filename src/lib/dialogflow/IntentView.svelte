<script>
  ...
  import {
    fetchAllIntents,
    buildIntentTree,
  } from "$lib/dialogflow/dialogflow-api.js";
  ...
  function handleWheel(e) {
    ...
    const newScale = Math.min(
      Math.max(oldScale * zoomFactor, MIN_ZOOM),
      MAX_ZOOM
    );
  ...
  let currentTree = $derived(
    $intentViewMode === "summary" ? summaryTree : allTree
  );
  ...
  function saveCheckedIntents() {
    if (browser) {
      localStorage.setItem(
        "dialogflow_checked_intents",
        JSON.stringify([...checkedIntents])
      );
    }
  }
  ...
  let checkedCount = $derived(
    searchResults.filter((id) => checkedIntents.has(id)).length
  );
...
<div class="h-full flex flex-col overflow-hidden relative">
  ...
  <div
    class="flex-shrink-0 flex flex-col gap-2 p-3 bg-white dark:bg-[#1a1a1a] border-b border-gray-200 dark:border-gray-800 z-10 shadow-sm"
  >
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div class="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5">
          <button
            ...
            class="px-3 py-1 text-xs font-medium rounded transition-colors {$intentViewMode ===
            'summary'
        ...
        <!-- Search Mode Toggle -->
        <div
          class="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5 px-2"
        >
          <span class="text-[10px] uppercase text-gray-500 font-bold mr-1"
            >Search:</span
          >
          <button
            onclick={() => {
            ...
            class="text-xs px-2 py-0.5 rounded {searchMode === 'standard'
              ? 'bg-white shadow text-blue-600'
              : 'text-gray-500'}">Std</button
          >
          <button
            onclick={() => {
              searchMode = "custom";
            ...
            class="text-xs px-2 py-0.5 rounded {searchMode === 'custom'
              ? 'bg-white shadow text-purple-600'
              : 'text-gray-500'}">Custom</button
          >
        </div>
      </div>
      ...
      >
        <button
          onclick={zoomOut}
          class="w-6 h-6 flex items-center justify-center hover:bg-white dark:hover:bg-gray-700 rounded"
          title="Zoom Out">-</button
        >
        <span class="text-xs w-8 text-center text-gray-600 dark:text-gray-400"
        ...
        <button
          onclick={resetZoom}
          class="text-xs px-2 hover:bg-white dark:hover:bg-gray-700 rounded"
    ...
    {#if searchMode === "custom"}
      <div
        class="flex items-center gap-2 mt-1 animate-in fade-in slide-in-from-top-1"
      >
        ...
        <div class="flex gap-0.5">
          <button
            ...
            class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded disabled:opacity-30"
    ...
    {#if searchResults.length > 0}
      <div class="relative z-20 ml-1">
        <div class="relative">
          ...
          >
            <span
              class="w-2 h-2 rounded-full {checkedCount === searchResults.length
                ? 'bg-green-500'
                : 'bg-purple-500'}"
            ></span>
            <span>{checkedCount} / {searchResults.length} Checked</span>
            <svg
              class="w-4 h-4 text-purple-400 group-hover:text-purple-600 transition-transform {showSearchResultsList
                ? 'rotate-180'
                : ''}"
              fill="none"
              stroke="currentColor"
          ...
          {#if showSearchResultsList}
            <div
              class="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-lg shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 max-h-[60vh] flex flex-col"
            >
              <div
                class="p-2 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50"
              >
              ...
              <div class="overflow-y-auto p-1 flex-1">
                {#each searchResults as id, index}
                  {@const node = findNodeById(currentTree.roots, id)}
                  <div
                    class="flex items-center gap-1 group/item p-1 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded"
                  >
                    <!-- Checkbox (syncs with checkedIntents) -->
                    <button
                      class="p-1.5 rounded text-gray-400 hover:text-green-500 hover:bg-gray-100 dark:hover:bg-gray-700 {checkedIntents.has(
                        id
                      )
                        ? 'text-green-500'
                        : ''}"
                    ...
                    <button
                      ...
                      class="flex-1 text-left text-xs px-1 py-1 truncate {index ===
                      currentResultIndex
      ...
      <div class="absolute top-16 right-4 z-50">
        <div class="relative">
          ...
          >
            ...
            <svg
              class="w-4 h-4 text-gray-400 group-hover:text-gray-600 transition-transform {showChecklist
                ? 'rotate-180'
                : ''}"
              fill="none"
              stroke="currentColor"
          ...
          {#if showChecklist}
            <div
              class="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-lg shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2"
            >
              <div
                class="p-2 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50"
              >
                <span class="text-xs font-bold text-gray-500 uppercase"
                  >Checked Intents</span
                >
                <button
                  onclick={resetChecklist}
                  class="text-xs text-red-500 hover:text-red-600 font-medium px-2 py-0.5 rounded hover:bg-red-50 dark:hover:bg-red-900/20"
                >
                  Reset
                </button>
              </div>
  ...
  <div class="flex-1 flex overflow-hidden">
    ...
    <div
      class="flex-1 overflow-auto bg-gray-50 dark:bg-[#0a0a0a] relative"
      bind:this={zoomContainer}
      onwheel={handleWheel}
    >
      <div
        class="min-w-full min-h-full p-8 origin-top-left"
        style="transform: scale({scale}); width: {100 / scale}%; height: {100 /
          scale}%;"
      ...
      >
        {#if $allIntents.length === 0}
          ...
          >
            <p>Loading intents...</p>
            <button
              onclick={() => fetchAllIntents()}
              class="mt-4 text-blue-500 hover:underline">Retry</button
            >
          </div>
        {:else if currentTree && currentTree.roots.length > 0}
          <div
            class="flex flex-row flex-wrap gap-8 justify-center items-start min-w-max pb-20"
          >
